...
this.npc.handleCollisionsWith(this.player, () => {
    this.dialogBox.displayText(
        "Ah... enfin un visiteur.",
        async () => {
            await new Promise((resolve) => setTimeout(resolve, 2000));
            this.dialogBox.clearText();

            this.dialogBox.displayText(
                `Sois le bienvenu, jeune voyageur. 
Comment puis -je vous aider?`,
                async () => {
                    ...
                    this.dialogBox.displayText(
                        `Ainsi, tu cherches à accéder aux archives 
sacrées du développeur Saad Ait Zaoui… ? `,
                        async () => {
                            await new Promise((resolve) => setTimeout(resolve, 2500));
                            this.dialogBox.clearText();

                            this.dialogBox.displayText(
                                `Mais tout savoir a un prix... Etes-vous 
prêt à prouver votre valeur?`,
                                async () => {
                                    ...
                                    this.dialogBox.displayText(
                                        `Seuls les guerriers courageaux peuvent 
