  ...
  return {
    ...
    draw() {
      ...
      if (
        ...
        this.dialogBox.clearText();
        this.dialogBox.displayText(
          `${this.playerPokemon.name} utilise ${this.playerPokemon.selectedAttack.name} !`,
          async () => {
      ...
      if (this.currentState === states.npcTurn && !this.npcPokemon.isFainted) {
        ...
        this.dialogBox.clearText();
        this.dialogBox.displayText(
          `${this.npcPokemon.name} lance ${this.npcPokemon.selectedAttack.name} !`,
          async () => {
...
if (this.currentState === states.battleEnd) {
  if (this.npcPokemon.isFainted) {
    this.dialogBox.clearText();
    this.dialogBox.displayText(
      `${this.npcPokemon.name} est tombé ! Vous gagner !`,
      async () => {
        await new Promise((resolve) => setTimeout(resolve, 1500));
        this.npc.x = 350;
        this.npc.y = 20;
        this.dialogBox.clearText();
        this.dialogBox.displayText(
          `Bien joué ! Tu es un guerrier courageux!
la voici votre récompense !`,
          async () => {
            // ⏱️ Attente de 2 secondes avant la redirection
            await new Promise((resolve) => setTimeout(resolve, 2000));
            window.location.href = "3DPortfolio/index.html"; // <-- Change ça par le nom exact de ta scène suivante
          }
        );
      }
    );
